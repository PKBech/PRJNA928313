...
ggsave(file="Figure_4A.svg", plot=Figure_4A, width=6.5, height=6.5)
...
# #Early and Peak 
# subset_taxa(Phylo_metabolome, mass %in% c(Early_Peak_unique) ) %>% 
#   subset_samples(., Phase != c("Late") ) %>%
#   #filter_taxa(., "mass" >= 300) %>%
#   filter_taxa(., function (x) {sum(x > 0) > 0}, prune=TRUE) %>%
#   #transform_sample_counts(., function(x) x/sum(x)) %>%  
#   plot_bar(., "sample_name_2", fill="mass") + facet_grid(NPC.pathway~Day, scales="free")
...
PERMANOVA <- adonis2(Metabolome_pseudo_counts_clr_dat_dist_eucledian ~ Day * Phase * Replicate, metadata_pseudo_counts_clr_dat_dist_eucledian)
PERMANOVA
...
#Alpha-diversity #####
#Phylo_metabolome <- subset_taxa(Phylo_metabolome, mass > 300)
...
#All NPC pathways aplha-analysis
aov_Observed_total_metabolome_dat <- Observed_total_metabolome_dat %>% 
  dplyr::group_by(NPC.pathway) %>% aov(Obs_features ~ as.factor(Day), .)

plot(aov_Observed_total_metabolome_dat,2) 
#Only for the Amino acids and Peptides
aov_Observed_total_metabolome_dat_NRPs <- Observed_total_metabolome_dat %>% 
  filter(NPC.pathway == "Amino acids and Peptides") %>% 
  #dplyr::group_by(NPC.pathway) %>% 
  aov(Obs_features ~ as.factor(Day), .)

plot(aov_Observed_total_metabolome_dat_NRPs,2) 

aov_Observed_total_metabolome_dat_NRPs %>%
  #dplyr::group_by(NPC.pathway) %>% 
  ggplot(aes(x=Obs_features)) + geom_histogram()

#Data is not normal distributed move to Kruskal-Wallis rank sum test, when ANNOVA assumptions are not met.


library(dplyr)
#Use Kruskal-Wallis rank sum test
results_kruskal.test <- by(Observed_total_metabolome_dat, Observed_total_metabolome_dat$NPC.pathway, 
              function(x) {
                test <- kruskal.test(x$Obs_features ~ as.factor(x$Day))
                data.frame(p.value = test$p.value, statistic = test$statistic)
              })

results_kruskal.test
...
dunnTest_results_perPhase <- list()
...
for (i in 1:length(groups)) {
  ...
  group_data <- subset(Observed_total_metabolome_dat, Observed_total_metabolome_dat$NPC.pathway == groups[i])
  # Perform Dunn test for current group
  test_result <- dunnTest(Obs_features ~ as.factor(Phase_major), data = group_data, method = "bh")
  # Store dunnTest_results in list with group name as name if P.adj is less than 0.05
  dunnTest_results_perPhase[[groups[i]]] <- test_result
  names(dunnTest_results_perPhase)[i] <- as.character(groups[i])
}

#all significant observations
dunnTest_results_perDay_filtered <- lapply(dunnTest_results_perDay, function(x) {
  data.frame(p.value = x$res$P.adj, Comparison = x$res$Comparison) %>%
    filter(p.value < 0.05)
})

dunnTest_results_perDay_filtered

dunnTest_results_perPhase_filtered <- lapply(dunnTest_results_perPhase, function(x) {
  data.frame(Z = x$res$Z, p.value = x$res$P.adj, Comparison = x$res$Comparison) %>%
    filter(p.value < 0.05)
})

dunnTest_results_perPhase_filtered
...
ggsave(file="Figure_4C.svg", plot=Figure_4C, width=11, height=6.5)
...
Phylo_metabolome_metadata$m.z <- rownames(Phylo_metabolome_metadata) 
#join with metadata table
results_kruskal.test <- left_join(results_kruskal.test, Phylo_metabolome_metadata) 



results_kruskal.test_sig <- results_kruskal.test %>% filter(p.value < 0.05)




results_kruskal.test_sig %>% group_by(Phase_major, NPC.pathway) %>% 
dplyr::summarise(n=n(), m.z = m.z)

otu_table(Phylo_metabolome) <- abs(otu_table(Phylo_metabolome))

subset_taxa(Phylo_metabolome, mass %in% results_kruskal.test_sig$m.z) %>% 
  #subset_taxa(., mass > 300) %>% 
  #subset_taxa(., NPC.pathway != "Amino acids and Peptides" & NPC.pathway != "Amino acids and Peptides") %>% 
  #subset_samples(., Phase == "Early") %>%
  #filter_taxa(., function (x) {sum(x > 0) > 10}, prune=TRUE) %>%
  #transform_sample_counts(., function(x) x/sum(x)) %>%  
  plot_bar(., "sample_name_2", fill="mass") + 
  facet_grid(NPC.pathway~Day, scales="free") + 
  theme(
    legend.position = "none", 
...
#Differential analysis unfilt######
Phylo_metabolome_pseudo_counts_unfilt_clr_dat <- as.data.frame(clr(Phylo_metabolome_pseudo_counts_unfilt))


Phylo_metabolome_pseudo_counts_unfilt_clr_dat_2 <- as.data.frame(lapply(as.data.frame(Phylo_metabolome_pseudo_counts_unfilt_clr_dat), function(x) ifelse(x < 0, 0, x)))
rownames(Phylo_metabolome_pseudo_counts_unfilt_clr_dat_2) <- rownames(Phylo_metabolome_pseudo_counts_unfilt_clr_dat)
colnames(Phylo_metabolome_pseudo_counts_unfilt_clr_dat_2) <- colnames(Phylo_metabolome_pseudo_counts_unfilt_clr_dat)

Phylo_metabolome_pseudo_counts_unfilt_clr_dat_2 <- (Phylo_metabolome_pseudo_counts_unfilt_clr_dat_2*100000)+1
#create new phyloseq object
Phylo_metabolome_pseudo_counts_unfilt_clr_dat <- phyloseq(otu_table(Phylo_metabolome_pseudo_counts_unfilt_clr_dat_2, taxa_are_rows = TRUE), 
                                               tax_table(as.matrix(metabolome_annotations_table_1)), sample_data(Phylo_metabolome))


ds_metabolome <- phyloseq_to_deseq2(Phylo_metabolome_pseudo_counts_unfilt_clr_dat, ~ Phase_major)


dds_metabolome <- DESeq(ds_metabolome)

deseq.results_metabolome <- na.omit(as.data.frame(results(dds_metabolome)))

library(EnhancedVolcano)
EnhancedVolcano(deseq.results_metabolome,
                lab = rownames(deseq.results_metabolome),
                x = 'log2FoldChange',
                y = 'padj',
...
deseq.results_metabolome_significant <- deseq.results_metabolome[deseq.results_metabolome$padj <= 10e-3 & 
                                                                   abs(deseq.results_metabolome$log2FoldChange) > 1, ]
deseq.results_metabolome_significant$mass <- rownames(deseq.results_metabolome_significant)

deseq.results_metabolome_significant$mass

#Significant features in Early vs. Late
subset_taxa(Phylo_metabolome, mass %in% deseq.results_metabolome_significant$mass & mass > 300) %>% 
  #subset_samples(., Phase == "Early") %>%
  filter_taxa(., function (x) {sum(x > 0) > 10}, prune=TRUE) %>%
  #transform_sample_counts(., function(x) x/sum(x)) %>%  
  plot_bar(., "sample_name_2", fill="mass") + 
  facet_grid(NPC.superclass~Day, scales="free") + 
  theme(legend.position = "none", axis.text.x=element_blank())

#Significant features in Early vs. Late
subset_taxa(Phylo_metabolome, mass %in% deseq.results_metabolome_significant$mass & NPC.pathway == "Polyketides") %>% 
   #subset_samples(., Phase == "Early") %>%
  filter_taxa(., function (x) {sum(x > 0) > 0}, prune=TRUE) %>%
  #transform_sample_counts(., function(x) x/sum(x)) %>%  
  plot_bar(., "sample_name_2", fill="mass") + 
...
#Deseq2 Differential analysis filtering######
Phylo_metabolome_filter <- filter_taxa(Phylo_metabolome, function (x) {sum(x > 0) > 10}, prune=TRUE)

#Impute pseudo count for zero's
Phylo_metabolome_filter_pseudo_counts <- cmultRepl(as.data.frame(otu_table(Phylo_metabolome_filter)), z.warning = 0.9)
#Phylo_metabolome_pseudo_counts_unfilt <- cmultRepl(as.data.frame(otu_table(Phylo_metabolome)), z.warning = 0.9)

#Centered log ratio transform
library(compositions)
Phylo_metabolome_filter_pseudo_counts_clr_dat <- as.data.frame(clr(Phylo_metabolome_filter_pseudo_counts))


Phylo_metabolome_filter_pseudo_counts_clr_dat_2 <- as.data.frame(lapply(as.data.frame(Phylo_metabolome_filter_pseudo_counts_clr_dat), function(x) ifelse(x < 0, 0, x)))
rownames(Phylo_metabolome_filter_pseudo_counts_clr_dat_2) <- rownames(Phylo_metabolome_filter_pseudo_counts_clr_dat)
colnames(Phylo_metabolome_filter_pseudo_counts_clr_dat_2) <- colnames(Phylo_metabolome_filter_pseudo_counts_clr_dat)

Phylo_metabolome_filter_pseudo_counts_clr_dat_2 <- (Phylo_metabolome_filter_pseudo_counts_clr_dat_2*100000)+1
#create new phyloseq object
Phylo_metabolome_filter_pseudo_counts_clr_dat_2_phylo <- phyloseq(otu_table(Phylo_metabolome_filter_pseudo_counts_clr_dat_2, taxa_are_rows = TRUE), 
                                                          tax_table(Phylo_metabolome_filter), sample_data(Phylo_metabolome_filter))


ds_metabolome <- phyloseq_to_deseq2(Phylo_metabolome_filter_pseudo_counts_clr_dat_2_phylo, ~ Phase_major)
...
deseq.results_metabolome_significant <- deseq.results_metabolome[deseq.results_metabolome$padj <= 10e-4 & 
                                                                   abs(deseq.results_metabolome$log2FoldChange) > 2, ]
deseq.results_metabolome_significant$mass <- rownames(deseq.results_metabolome_significant)

deseq.results_metabolome_significant$mass

deseq.results_metabolome$mass <- rownames(deseq.results_metabolome)

deseq.results_metabolome <- left_join(deseq.results_metabolome, data.frame(tax_table(Phylo_metabolome_filter)))
EnhancedVolcano(deseq.results_metabolome,
                lab = deseq.results_metabolome$mass,
                x = 'log2FoldChange',
                y = 'padj',
                #selectLab = c('2-keto-4-pentenoate hydratase (MhpD) (PDB:1SV6)'),
                pCutoff = 10e-2,
...
#524.37393 - Fatty acid - Glycerophosphocholine 
#579.46069 - Fatty acid - Polyamine - 

#Significant features in Early vs. Late
subset_taxa(Phylo_metabolome_filter, mass %in% deseq.results_metabolome_significant$mass) %>% 
  subset_taxa(., mass > 300) %>% 
  #subset_taxa(., NPC.pathway != "Unclassified") %>% 
  #subset_samples(., Phase == "Early") %>%
  #filter_taxa(., function (x) {sum(x > 0) > 10}, prune=TRUE) %>%
  #transform_sample_counts(., function(x) x/sum(x)) %>%  
  plot_bar(., "sample_name_2", fill="mass") + 
...
#Significant features in Early vs. Late
subset_taxa(Phylo_metabolome, mass %in% deseq.results_metabolome_significant$mass) %>% 
  subset_taxa(., mass > 300) %>% 
  #subset_taxa(., NPC.pathway != "Unclassified") %>% 
  #subset_samples(., Phase == "Early") %>%
  #filter_taxa(., function (x) {sum(x > 0) > 10}, prune=TRUE) %>%
  transform_sample_counts(., function(x) x/sum(x)) %>%  
  plot_bar(., "sample_name_2", fill="NPC.superclass") + 
  facet_grid(~Day, scales="free") + 
  theme(
    #legend.position = "none", 
    axis.text.x=element_blank())

#ANCOM-BC Differential analysis filtering######
Phylo_metabolome_filter <- filter_taxa(Phylo_metabolome, function (x) {sum(x > 0) > 10}, prune=TRUE)

library(gmp)
library(tidyverse)
library(ANCOMBC)
Phylo_metabolome_filter_ANCOM <- ancombc(phyloseq = Phylo_metabolome_filter, formula = "Phase_major",
...
Phylo_metabolome_filter_ANCOM_lfc <- Phylo_metabolome_filter_ANCOM$res$lfc 
Phylo_metabolome_filter_ANCOM_lfc$sig <- Phylo_metabolome_filter_ANCOM$res$diff_abn$Phase_majorLate

Phylo_metabolome_filter_ANCOM_lfc_sig <- Phylo_metabolome_filter_ANCOM_lfc %>% filter(sig == TRUE)

#Significant features in Early vs. Late
subset_taxa(Phylo_metabolome_filter, mass %in% Phylo_metabolome_filter_ANCOM_lfc_sig$taxon) %>% 
  subset_taxa(., mass > 300) %>% 
  subset_taxa(., NPC.pathway == "Amino acids and Peptides") %>% 
  #subset_samples(., Phase == "Early") %>%
  #filter_taxa(., function (x) {sum(x > 0) > 10}, prune=TRUE) %>%
  #transform_sample_counts(., function(x) x/sum(x)) %>%  
  plot_bar(., "sample_name_2", fill="mass") + 
...
# subset_taxa(Phylo_metabolome_filter, mass %in% Phylo_metabolome_filter_ANCOM_lfc_sig$taxon) %>% 
#   subset_taxa(., mass > 300) %>% 
#   subset_taxa(., NPC.pathway == "Amino acids and Peptides") %>% otu_table(.) %>% rownames(.)

peptide_cluster_1 <- c("446.34805",
"496.36291",
"472.36388",
"708.57757",
"650.46037",
...
peptide_cluster_2 <- c("606.38681","733.48471",
"691.43956","732.46598",
"703.47589","698.48203",
"782.44497","762.47601",
"748.4609")

peptide_cluster_3 <- c("725.57645",
"800.60295",
"562.37458",
"668.50992",
"724.57248",
...
subset_taxa(Phylo_metabolome, mass %in% peptide_cluster_3) %>% 
  #subset_taxa(., mass > 300) %>% 
  #subset_taxa(., NPC.pathway != "Unclassified") %>% 
  #subset_samples(., Phase == "Early") %>%
  #filter_taxa(., function (x) {sum(x > 0) > 10}, prune=TRUE) %>%
...
subset_taxa(Phylo_metabolome_filter, mass %in% Phylo_metabolome_filter_ANCOM_lfc_sig$taxon) %>% 
  ...
  #filter_taxa(., function (x) {sum(x > 0) > 10}, prune=TRUE) %>%
  #transform_sample_counts(., function(x) x/sum(x)) %>%  
  plot_bar(., "sample_name_2", fill="NPC.class") + 
  facet_grid(NPC.pathway~Day, scales="free") + 
  theme(
    #legend.position = "none", 
    axis.text.x=element_blank())
...
subset_taxa(Phylo_metabolome_filter, mass %in% Phylo_metabolome_filter_ANCOM_lfc_sig$taxon) %>% 
  subset_taxa(., mass %in% peptide_cluster_2) %>% 
  #subset_taxa(., NPC.pathway == "Amino acids and Peptides") %>% 
  #subset_samples(., Phase == "Early") %>%
  #filter_taxa(., function (x) {sum(x > 0) > 10}, prune=TRUE) %>%
  #transform_sample_counts(., function(x) x/sum(x)) %>%  
...
as.data.frame(tax_table(Phylo_metabolome))$mass

430.2443
# Phylo_metabolome_filter_ANCOM_lfc_sig <- left_join(Phylo_metabolome_filter_ANCOM_lfc_sig, Phylo_metabolome_filter_ANCOM$res$se, by = "taxon")
# colnames(Phylo_metabolome_filter_ANCOM_lfc_sig)[5] <- "SD"
# 
# #### Dataframe Day 1
# df_ANCOM = data.frame(Phylo_metabolome_filter_ANCOM$res$lfc * Phylo_metabolome_filter_ANCOM$res$diff_abn) %>%
...
#SDAMS: Differential Abundant Analysis for Metabolomics, Proteomics and single-cell RNA sequencing Data #####
Phylo_metabolome_filter <- filter_taxa(Phylo_metabolome, function (x) {sum(x > 0) > 10}, prune=TRUE)

featureInfo <- (as.data.frame(otu_table(Phylo_metabolome)))
#featureInfo <- Phylo_metabolome_pseudo_counts


rownames(featureInfo)
featureInfo[featureInfo<0] <- 0

groupInfo <- (data.frame(sample_data(Phylo_metabolome)))[6]


exampleSE2 <- createSEFromMatrix(feature = featureInfo, colData = groupInfo)

head(colData(exampleSE2)$Phase_major)

results <- SDA(exampleSE2)

significant_features <- data.frame(feature = results$feat.names, pval = results$pv_beta, beta = results$beta)
significant_features <- na.omit(significant_features)
significant_features$pval <- as.numeric(significant_features$pval)
significant_features$beta <- as.numeric(significant_features$beta)

significant_features <- significant_features[significant_features$pval<0.05,]
significant_features <- significant_features[significant_features$pval<0.05,]$feature


subset_taxa(Phylo_metabolome, mass %in% significant_features) %>% 
  subset_taxa(., mass > 300) %>% 
  #subset_taxa(., NPC.pathway != "Unclassified") %>% 
  #subset_samples(., Phase == "Early") %>%
  #filter_taxa(., function (x) {sum(x > 0) > 10}, prune=TRUE) %>%
...
subset_taxa(Phylo_metabolome) %>% 
  #subset_taxa(., mass > 300) %>% 
  subset_taxa(., NPC.pathway == "Amino acids and Peptides") %>% 
  #subset_taxa(., NPC.superclass == "β-lactams") %>% 
  #subset_samples(., sample_name_2 == "Succession-T9-9") %>%
  #filter_taxa(., function (x) {sum(x > 0) > 10}, prune=TRUE) %>%
  #transform_sample_counts(., function(x) x/sum(x)) %>%  
  plot_bar(., "sample_name_2", fill="NPC.superclass") + 
  facet_grid(~Day, scales="free") + 
...
subset_taxa(Phylo_metabolome) %>% 
  subset_taxa(., mass > 300) %>% 
  subset_taxa(., NPC.superclass == "β-lactams") %>% 
  subset_samples(., sample_name_2 == "Succession-T9-9") %>% tax_table(.) %>% as.data.frame(.)
...
###Filtering out features that is not present in more than 10 samples across all samples
...
#Barlots composition on filtered data for overall trends ####
#Quick look at barplot with classifications before filtering an normalization of the data
plot_bar(Phylo_metabolome_filter_norm, "sample_name_2", fill="NPC.pathway") + facet_grid(~Day, scales="free_x")
...
subset_taxa(Phylo_metabolome_filter, NPC.pathway == "Polyketides") %>% plot_bar("sample_name_2", fill="mass") + 
  facet_grid(NPC.pathway~factor(Day), scales="free") 
as.data.frame(tax_table(subset_taxa(Phylo_metabolome_filter, NPC.pathway == "Polyketides") ))[,1:6]
#300.21699 300.21699 Polyketide prominant for the early phase

subset_taxa(Phylo_metabolome_filter, NPC.pathway == "Terpenoids") %>% 
  transform_sample_counts(., function(x) x/sum(x)) %>%
  plot_bar("sample_name_2", fill="mass") + 
  facet_grid(NPC.class~factor(Day), scales="free") 
as.data.frame(tax_table(subset_taxa(Phylo_metabolome_filter, NPC.pathway == "Terpenoids") ))[,1:7]
#609.471   Baccharane triterpenoids prominant for early phase but likely a plant NP

subset_taxa(Phylo_metabolome, NPC.pathway == "Shikimates and Phenylpropanoids") %>% plot_bar("sample_name_2", fill="mass") + 
  facet_grid(NPC.class~factor(Day), scales="free") 
as.data.frame(tax_table(subset_taxa(Phylo_metabolome_filter, NPC.pathway == "Shikimates and Phenylpropanoids") ))[,1:6]
#Phenylpropanoids at day 29 and 44: worth looking up, too small mass to be something interesting
subset_taxa(Phylo_metabolome, mass == "122.09631") %>% plot_bar("sample_name_2", fill="mass") + 
  facet_grid(NPC.class~factor(Day), scales="free") 
as.data.frame(tax_table(subset_taxa(Phylo_metabolome_filter, NPC.pathway == "Shikimates and Phenylpropanoids") ))[,1:6]
...
subset_taxa(Phylo_metabolome_filter, NPC.pathway == "Amino acids and Peptides") %>% plot_bar("sample_name_2", fill="mass") + 
  facet_grid(NPC.class~factor(Day), scales="free") 
as.data.frame(tax_table(subset_taxa(Phylo_metabolome, NPC.pathway == "Amino acids and Peptides") ))[,1:6]
#Most present in late phase

# subset_taxa(Phylo_metabolome_filter, NPC.pathway == "Amino acids and Peptides" & mass >= 300) %>% plot_bar("sample_name_2", fill="mass") + 
#   facet_grid(NPC.~factor(Day), scales="free") 
# #macrolides mass: 505.2542 common in late phase

subset_taxa(Phylo_metabolome_filter, NPC.pathway == "Fatty acids") %>% plot_bar("sample_name_2", fill="mass") + 
  facet_grid(NPC.class~factor(Day), scales="free") 
#No patterns 

subset_taxa(Phylo_metabolome_filter, NPC.pathway == "Unclassified") %>% 
  transform_sample_counts(., function(x) x/sum(x)) %>%
  plot_bar("sample_name_2", fill="mass") + 
  facet_grid(NPC.class~factor(Day), scales="free") 
as.data.frame(tax_table(subset_taxa(Phylo_metabolome, NPC.pathway == "Amino acids and Peptides") ))[,1:6]
...
openSession(file.location = "Figure5-Metabolome/Jyllinger harbor_no_sirius_annotations260123.cys")#
Sys.sleep(1)
#setCurrentView("The cytoscape view you are working on")
nodetab=getTableColumns(table = "node")
nodeedges=getTableColumns(table = "edge")
nodeNetWork=getTableColumns(table = "network")
str(nodetab)
# colnames(SERIUS_table)[1] <- "name"
# SERIUS_table[1] <- as.character(SERIUS_table[1])
# str(SERIUS_table[1])
# str(nodetab[2])
# str(met_table)

metadata_table <- cbind((met_table[,100:118])[,c(3,5,7)], as.character(met_table$row.ID), rownames(met_table))


colnames(metadata_table) <- c("NCP_pathway", "NCP_superclass","NCP_class",  "shared name", "parent mass")

nodetab_names <- data.frame(nodetab$`shared name`, nodetab$`parent mass`)
colnames(nodetab_names) <- c("shared name", "parent mass")


nodetab_new <- left_join(nodetab_names, metadata_table, by = "shared name")

nodetab$NCP.pathway <- nodetab_new$NCP_pathway

length(nodetab$`shared name`)
length(nodetab$NCP.pathway)
# nodetab_new <- left_join(nodetab, metadata_table, by = "shared name")


nodetab_new[, c("shared name", "parent mass.x" , "parent mass.y" ,"NCP_pathway")]


nodetab_new[]
rownames(metadata_table) <- metadata_table$`shared name`
...
createNetworkFromDataFrames(nodetab,nodeedges, title="modified_1", collection="DataFrame")

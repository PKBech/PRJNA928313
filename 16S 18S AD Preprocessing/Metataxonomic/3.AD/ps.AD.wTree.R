### Phylogenetic tree and AD

# Phyloseq object to use (cleaned) 
ps_AD <- readRDS("16S 18S AD Preprocessing/Metataxonomic/3.AD/ps_AD.rds")

#Make DNAbin
seqs = as.DNAbin(refseq(ps_AD))
...
table(nchar(as.character(refseq(ps_AD)))) # check length 
...
system2("/path/to/FastTree", args = c("-gtr", "-nt", "-out", "asv_aligned.tree", "asv_aligned_correctnames.fasta"))
...
phy_tree(ps_AD) <- phy_tree(treeNewR)
ps_AD

#Save PS with tree
save(ps_AD, file = "16S 18S AD Preprocessing/Metataxonomic/3.AD/ps.AD.wTree.RData")

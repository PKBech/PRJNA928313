...
load("AmpliconAnalysis/16S/ps.ASV.reduced.RData")
ps.ASV.reduced.16S
#Make DNAbin
seqs = as.DNAbin(refseq(ps.ASV.reduced.16S))
...
table(nchar(as.character(refseq(ps_16S)))) # check length 
...
phy_tree(ps.ASV.reduced.16S) <- phy_tree(treeNewR)
ps.ASV.reduced.16S

#Save PS with tree
save(ps.ASV.reduced.16S, file = "AmpliconAnalysis/16S/ps_16S.reduced.wTree.20062022.RData")
